...
**已交付**（2026-07-20，3024 個 headless 測試全綠／零洩漏，測試總耗時約 11 秒）：

- **本機雙人**（主選單 → 選秀 BP → 對戰 → 終局統計）＋**單人 vs CPU**（白/紅/藍/綠/橙/Boss 六關卡 AI）。
- **連線對戰**（Phase 12，規格 `docs/rebuild/10_連線版本.md`）：專用伺服器＋房間制
  （開房邀請、公開/上鎖＋密碼、旁觀開關），**伺服器權威**、手牌公開（D19）、
  BP → 對戰 → 終局 → 再戰閉環、旁觀中途加入、斷線重連、終局回放下載。Ubuntu 已部署。
...
- **美術可接手**（Phase 14）：棋盤幾何可在編輯器拖曳、13 個 item 模板場景、theme 具名色、
  素材**丟檔案即生效**、38＋項特效參數可在 Inspector 調——詳見「美術協作」節。
- 平衡數值以 JSON 為單一來源，由 Python 原版同步。

**目前狀態**：自動開發任務已收官（Phase 15 程式碼品質收官完成）。
剩餘為人工驗收（實機跨機、視覺體感、編輯器實測）與 P9-4 UI 美術優化
（使用者帶方向的持續型）。爬塔（Phase 13）擱置中。
