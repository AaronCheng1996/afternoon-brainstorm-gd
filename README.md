...
**已交付：本機雙人（hot-seat）**——主選單 → 選秀 BP → 對戰 → 終局統計 → 回選單，846 個 headless 測試全綠。

- 10 色 × 8 職業全卡牌 + 特殊卡/衍生物（能力系統，含沉默/附魔）。
- 佔位美術（幾何圖形＋文字）＋動畫插槽（待機/攻擊/投射物/受擊/死亡/施法）。
- 平衡數值以 JSON 為單一來源，由 Python 原版同步。
- UI 骨架宣告於 `.tscn`（編輯器可視可編輯，美術可接手）。

**進行中／排程（詳見 `docs/rebuild/06_任務清單.md`）**：
Phase 8 UI/UX 基座（多分辨率、Theme、關鍵字提示、百科字典、記分板、統計強化）→
Phase 9 斜 45° 棋盤與演出美術 → Phase 10 CPU AI（單人對戰）→
Phase 11 計時器與回放 → Phase 12 連線版本。爬塔（Phase 13）擱置中。
...
`docs/rebuild/`：`00_總覽`（目標/鐵則/重大決策）、`01_遊戲規則規格`、`02_卡牌能力總表`、`03_CPU_AI規格`、`04_架構設計`、`05_JSON平衡同步`、`06_任務清單`（主執行清單）、`07_爬塔模式與新功能`、`08_場景編輯器化`、**`09_程式碼導覽`（每個檔案的職責說明，review 入口）**、`歸檔_已完成任務`、`進度日誌`。人工驗收檢查表：`驗收_對戰.md`、`驗收_BP.md`、`驗收_選單流程.md`。
