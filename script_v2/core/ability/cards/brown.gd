...
# 這是 v2 內建沉默（silence）機制的第一個實際使用者（見 04 §5.5、D3）。
#
# 沉默模型：
...
static func registrations() -> Dictionary:
	return {
		"ADCBR": Callable(BrownCardsV2, "adcbr"),
		"APBR": Callable(BrownCardsV2, "apbr"),
		"TANKBR": Callable(BrownCardsV2, "tankbr"),
		"HFBR": Callable(BrownCardsV2, "hfbr"),
		"LFBR": Callable(BrownCardsV2, "lfbr"),
		"ASSBR": Callable(BrownCardsV2, "assbr"),
		"APTBR": Callable(BrownCardsV2, "aptbr"),
		"SPBR": Callable(BrownCardsV2, "spbr"),
	}


# 每張 Brown 卡共用的「還原沉默」能力（不受沉默，優先權最低使其先於同觸發能力執行）。
static func _restore() -> AbilityV2:
	var tags: Array[int] = [AbilityComponentV2.Tag.PASSIVE]
	return AbilityV2.new("brown_restore", TriggerV2.Type.ON_REFRESH, [RestoreEffect.new()], tags, -100)


# ADCBR：攻擊後自身麻痺（effects_off 時失效 → 沉默即改回普通攻擊）。
static func adcbr() -> Array:
	var f: Array[int] = [AbilityComponentV2.Tag.FACTION]
	return [
		_restore(),
		AbilityV2.new("brown_adc_attack", TriggerV2.Type.ATTACK_OVERRIDE, [AdcAttackEffect.new()], f),
	]
...
static func apbr() -> Array:
	...
	return [
		_restore(),
		AbilityV2.new("brown_ap_draw", TriggerV2.Type.ON_ABILITY_HIT, [ApDrawEffect.new()], f),
	]
...
static func tankbr() -> Array:
	...
	return [
		_restore(),
		AbilityV2.new("brown_tank_flag", TriggerV2.Type.ON_BEEN_ATTACKED, [TankFlagEffect.new()], f),
		AbilityV2.new("brown_tank_decay", TriggerV2.Type.ON_REFRESH, [TankDecayEffect.new()], f),
	]
...
static func hfbr() -> Array:
	...
	return [
		_restore(),
		AbilityV2.new("brown_hf_attack", TriggerV2.Type.ATTACK_OVERRIDE, [HfAttackEffect.new()], tags),
	]
...
static func lfbr() -> Array:
	...
	return [
		_restore(),
		AbilityV2.new("brown_lf_kill", TriggerV2.Type.ON_KILLED, [LfKillEffect.new()], f),
	]
...
static func assbr() -> Array:
	...
	return [
		_restore(),
		AbilityV2.new("brown_ass_kill", TriggerV2.Type.ON_KILLED, [AssKillEffect.new()], f),
	]
...
static func aptbr() -> Array:
	...
	return [
		_restore(),
		AbilityV2.new("brown_apt_deploy", TriggerV2.Type.ON_DEPLOY, [AptDeployEffect.new()], passive),
		AbilityV2.new("brown_apt_buff", TriggerV2.Type.ON_ABILITY_HIT, [AptBuffEffect.new()], f),
	]
...
static func spbr() -> Array:
	...
	return [
		_restore(),
		AbilityV2.new("brown_sp_silence", TriggerV2.Type.ON_ABILITY_HIT, [SpSilenceEffect.new()], tags),
	]
...
class ApDrawEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var core: GameCore = ctx.core
		var src: PieceState = ctx.source
...
class TankFlagEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
...
class TankDecayEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var core: GameCore = ctx.core
		var src: PieceState = ctx.source
		...
		if not was_attacked:
			...
			CombatV2.damage_calculate(core, src, loss, src, false, 0.0)
			src.hit_cards.clear()
...
class HfAttackEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var core: GameCore = ctx.core
		var src: PieceState = ctx.source
		var ok: bool = CombatV2.launch_attack(core, src, src.attack_types)
		...
		var silenced: bool = src.abilities != null \
			and src.abilities.silenced_tags.has(AbilityComponentV2.Tag.FACTION)
		src.attack_uses = 1 if silenced else 2
		return ok


# LFBR：斬殺後敵方得 on_kill_enemy_points 分（player1 擁有 → score 增＝對面得分）。
class LfKillEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var core: GameCore = ctx.core
		var src: PieceState = ctx.source
...
# ASSBR：斬殺後我方跳過下回合抽牌。
class AssKillEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var core: GameCore = ctx.core
...
class AptDeployEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var core: GameCore = ctx.core
		var src: PieceState = ctx.source
...
class AptBuffEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var core: GameCore = ctx.core
		var src: PieceState = ctx.source
		...
		for ally: PieceState in BrownCardsV2._nearest_allies(core, src):
...
class SpSilenceEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var core: GameCore = ctx.core
		var src: PieceState = ctx.source
		src.set_anger(true)
		for c: PieceState in BrownCardsV2._other_browns(core, src):
			if c.abilities != null:
				c.abilities.silence_tag(AbilityComponentV2.Tag.FACTION)
		return true
