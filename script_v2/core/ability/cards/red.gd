...
static func registrations() -> Dictionary:
	return {
		"ADCR": Callable(RedCardsV2, "adcr"),
		"APR": Callable(RedCardsV2, "apr"),
		"TANKR": Callable(RedCardsV2, "tankr"),
		"HFR": Callable(RedCardsV2, "hfr"),
		"LFR": Callable(RedCardsV2, "lfr"),
		"ASSR": Callable(RedCardsV2, "assr"),
		"APTR": Callable(RedCardsV2, "aptr"),
		"SPR": Callable(RedCardsV2, "spr"),
	}
...
static func adcr() -> Array:
	...
	return [AbilityV2.new("red_adc_grow", TriggerV2.Type.ON_ABILITY_HIT, [AdcGrowEffect.new()], tags)]


# APR：攻擊附帶麻痺 + 偷取目標 100% ATK（自身與 SPR +偷取值，目標 ATK 歸 0）（Python Ap.ability）。
static func apr() -> Array:
	var tags: Array[int] = [AbilityComponentV2.Tag.TRIGGERED]
	return [AbilityV2.new("red_ap_steal", TriggerV2.Type.ON_ABILITY_HIT, [ApStealEffect.new()], tags)]


# TANKR：被攻擊後 最近友方（不含自己）+2 護盾；SPR +2 護盾（Python Tank.been_attacked）。
static func tankr() -> Array:
	var tags: Array[int] = [AbilityComponentV2.Tag.TRIGGERED]
	return [AbilityV2.new("red_tank_armor", TriggerV2.Type.ON_BEEN_ATTACKED, [TankArmorEffect.new()], tags)]
...
static func hfr() -> Array:
	...
	return [
		AbilityV2.new("red_hf_grow", TriggerV2.Type.ON_ABILITY_HIT, [HfGrowEffect.new()], trig),
		AbilityV2.new("red_hf_immortal", TriggerV2.Type.CAN_BE_KILLED, [HfImmortalEffect.new()], mod),
		AbilityV2.new("red_hf_settle", TriggerV2.Type.ON_SETTLE, [HfSettleEffect.new()], mod),
	]
...
static func lfr() -> Array:
	var tags: Array[int] = [AbilityComponentV2.Tag.TRIGGERED]
	return [AbilityV2.new("red_lf_grow", TriggerV2.Type.ON_ABILITY_HIT, [LfGrowEffect.new()], tags)]


# ASSR：斬殺後 最近友方 ATK+2；SPR +2（Python Ass.killed）。
static func assr() -> Array:
	var tags: Array[int] = [AbilityComponentV2.Tag.TRIGGERED]
	return [AbilityV2.new("red_ass_kill_buff", TriggerV2.Type.ON_KILLED, [AssKillBuffEffect.new()], tags)]


# APTR：攻擊時 最近友方 +1/+1，自己 +1/+1，SPR +1/+1（護盾/ATK）（Python Apt.ability）。
static func aptr() -> Array:
	var tags: Array[int] = [AbilityComponentV2.Tag.TRIGGERED]
	return [AbilityV2.new("red_apt_buff", TriggerV2.Type.ON_ABILITY_HIT, [AptBuffEffect.new()], tags)]
...
static func _nearest_allies(core: GameCore, source: PieceState) -> Array:
	...
	return CombatV2.detection(core, source, "nearest", allies)


# --- 效果 ---

# ADCR：自身 ATK+damage_increase，SPR 同步。
class AdcGrowEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		...
		src.damage += inc
		for sp: PieceState in RedCardsV2._allied_sprs(core, src.owner):
			sp.damage += inc
		return true


# APR：目標麻痺；偷取目標 attack_steal_rate% ATK，自身與 SPR +偷取值，目標 ATK -偷取值。
class ApStealEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var src: PieceState = ctx.source
...
class TankArmorEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var src: PieceState = ctx.source
		var core: GameCore = ctx.core
		var inc: int = int(core.balance.param(src.card_id, "armor_increase", 0))
		for ally: PieceState in RedCardsV2._nearest_allies(core, src):
			ally.armor += inc
		for sp: PieceState in RedCardsV2._allied_sprs(core, src.owner):
			sp.armor += inc
		return true
...
class HfGrowEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var src: PieceState = ctx.source
		var core: GameCore = ctx.core
		...
		core.event_sink.append(GameEventV2.hurt(src.pos(), 0.0, src.health))
		var inc: int = int(core.balance.param(src.card_id, "damage_increase", 0))
		src.damage += inc
		for sp: PieceState in RedCardsV2._allied_sprs(core, src.owner):
			sp.damage += inc
...
class HfImmortalEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
...
class LfGrowEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var src: PieceState = ctx.source
		var core: GameCore = ctx.core
...
class AssKillBuffEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var src: PieceState = ctx.source
		var core: GameCore = ctx.core
		var inc: int = int(core.balance.param(src.card_id, "damage_increase", 0))
