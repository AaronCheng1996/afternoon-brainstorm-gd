...
var _net_quality: String = ""       # P12-17：連線品質文字
...
var _phase_secs_shown: int = -1     # P15-2：階段列上已顯示的剩餘秒（僅跨秒時才重建標籤）
...
func _process(delta: float) -> void:
	...
	if _phase_timer.advance(delta):
		...
		# P15-2：標籤上唯一隨時間變的是剩餘秒，而它每秒才變一次 → 只在跨秒時重建
		# （原本逐幀重組整條階段列字串）。階段/張數等變動另有 _refresh 路徑，不受影響。
		var secs: int = _phase_timer.remaining_seconds()
		if secs != _phase_secs_shown:
			_phase_secs_shown = secs
			_update_phase_label()
...
func _net_phase_text() -> String:
	...
	if not _net_spectator:
		...
		if _net_opp_name != "":
			base += "　｜　對手：%s" % _net_opp_name
	if _net_remaining >= 0:
		base += "　｜　⏳ %d 秒" % _net_remaining
	if _net_rtt >= 0:
		base += "　｜　延遲 %d ms（%s）" % [_net_rtt, _net_quality]
