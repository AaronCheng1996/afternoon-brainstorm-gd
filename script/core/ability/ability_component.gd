...
var native: Array = []                # Array[Ability]：registry 依 card_id 組裝
var granted: Array = []               # Array[Ability]：附魔（動態掛上）
...
# 某能力目前是否有效（未被沉默）。
func is_ability_active(a: Ability) -> bool:
...
func _active_for(trigger: int, ctx: AbilityContext) -> Array:
	var matched: Array = []
	for a: Ability in all_abilities():
		...
		matched.append(a)
	if matched.size() > 1:
		matched.sort_custom(func(x: Ability, y: Ability) -> bool: return x.priority < y.priority)
	return matched


# 副作用型觸發（ON_KILLED / ON_BEEN_ATTACKED / ON_DEPLOY / ON_REFRESH / ON_DIE…）。
# 逐一執行效果；回傳是否有任一能力被觸發。
func run(trigger: int, ctx: AbilityContext) -> bool:
	var any: bool = false
	for a: Ability in _active_for(trigger, ctx):
		for e: AbilityEffect in a.effects:
			e.execute(ctx)
		any = true
	return any
...
func dispatch_mod(trigger: int, ctx: AbilityContext) -> int:
	for a: Ability in _active_for(trigger, ctx):
		for e: AbilityEffect in a.effects:
...
func any_true(trigger: int, ctx: AbilityContext) -> bool:
	for a: Ability in _active_for(trigger, ctx):
		for e: AbilityEffect in a.effects:
			var r: Variant = e.execute(ctx)
...
func dispatch_attack_override(ctx: AbilityContext) -> Variant:
	for a: Ability in _active_for(Trigger.Type.ATTACK_OVERRIDE, ctx):
		for e: AbilityEffect in a.effects:
			var r: Variant = e.execute(ctx)
...
func collect_field(ctx: AbilityContext) -> Array:
	var out: Array = []
	for a: Ability in _active_for(Trigger.Type.MOD_FIELD_INTERCEPT, ctx):
		for e: AbilityEffect in a.effects:
			var r: Variant = e.execute(ctx)
