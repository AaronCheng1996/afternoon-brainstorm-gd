...
# 呼叫 Combat.launch_attack(custom_targets)，所有攻擊鉤子（ON_ABILITY_HIT/ON_KILLED…）
# 因此自然落在本體上（對齊 Python Shadow.attack → self.linker.launch_attack(...)）。
#
# 每個 static func 回傳該 card_id 的 native 能力陣列（Array[Ability]）。
class_name FuchsiaCards
extends RefCounted


# 註冊表：card_id -> Callable（回傳 Array[Ability]）。含衍生物 SHADOW。
static func registrations() -> Dictionary:
	return {
		"ADCF": Callable(FuchsiaCards, "adcf"),
		"APF": Callable(FuchsiaCards, "apf"),
		"TANKF": Callable(FuchsiaCards, "tankf"),
		"HFF": Callable(FuchsiaCards, "hff"),
		"LFF": Callable(FuchsiaCards, "lff"),
		"ASSF": Callable(FuchsiaCards, "assf"),
		"APTF": Callable(FuchsiaCards, "aptf"),
		"SPF": Callable(FuchsiaCards, "spf"),
		"SHADOW": Callable(FuchsiaCards, "shadow"),
	}


# ADCF：佈署生鏡像；攻擊時本體與鏡像都發動（大十字）；本體無目標時只有鏡像打也算成功。
static func adcf() -> Array:
	var on_enter: Array[int] = [AbilityComponent.Tag.ON_ENTER]
	var passive: Array[int] = [AbilityComponent.Tag.PASSIVE]
	var trig: Array[int] = [AbilityComponent.Tag.TRIGGERED]
	return [
		Ability.new("fuchsia_adc_deploy", Trigger.Type.ON_DEPLOY, [DeploySpawnShadowEffect.new()], on_enter),
		Ability.new("fuchsia_adc_attack", Trigger.Type.ATTACK_OVERRIDE, [BodyPlusShadowAttackEffect.new()], passive),
		Ability.new("fuchsia_adc_mirror", Trigger.Type.ON_AFTER_MOVEMENT, [MirrorMoveEffect.new()], trig),
	]
...
static func apf() -> Array:
	...
	var trig: Array[int] = [AbilityComponent.Tag.TRIGGERED]
	return [
		Ability.new("fuchsia_ap_deploy", Trigger.Type.ON_DEPLOY,
			[DeploySpawnShadowEffect.new(), AuraNumbEffect.new()], on_enter),
		Ability.new("fuchsia_ap_aura", Trigger.Type.ON_REFRESH, [AuraNumbEffect.new()], trig),
		Ability.new("fuchsia_ap_numb", Trigger.Type.ON_ABILITY_HIT, [NumbTargetEffect.new()], trig),
		Ability.new("fuchsia_ap_mirror", Trigger.Type.ON_AFTER_MOVEMENT, [MirrorMoveEffect.new()], trig),
	]
...
static func tankf() -> Array:
	...
	var trig: Array[int] = [AbilityComponent.Tag.TRIGGERED]
	return [
		Ability.new("fuchsia_tank_deploy", Trigger.Type.ON_DEPLOY, [DeploySpawnShadowEffect.new()], on_enter),
		Ability.new("fuchsia_tank_occupy", Trigger.Type.ON_UPDATE, [OccupyShadowCellsEffect.new()], passive),
		Ability.new("fuchsia_tank_die", Trigger.Type.ON_DIE, [ReleaseShadowCellsEffect.new()], passive),
		Ability.new("fuchsia_tank_mirror", Trigger.Type.ON_AFTER_MOVEMENT, [MirrorMoveEffect.new()], trig),
	]
...
static func hff() -> Array:
	...
	var trig: Array[int] = [AbilityComponent.Tag.TRIGGERED]
	return [
		Ability.new("fuchsia_hf_deploy", Trigger.Type.ON_DEPLOY, [DeploySpawnShadowEffect.new()], on_enter),
		Ability.new("fuchsia_hf_attack", Trigger.Type.ATTACK_OVERRIDE, [BodyPlusShadowAttackEffect.new()], passive),
		Ability.new("fuchsia_hf_mirror", Trigger.Type.ON_AFTER_MOVEMENT, [MirrorMoveEffect.new()], trig),
	]
...
static func lff() -> Array:
	...
	var trig: Array[int] = [AbilityComponent.Tag.TRIGGERED]
	return [
		Ability.new("fuchsia_lf_deploy", Trigger.Type.ON_DEPLOY, [DeploySpawnShadowEffect.new("nearest")], on_enter),
		Ability.new("fuchsia_lf_attack", Trigger.Type.ATTACK_OVERRIDE, [LfBodyShadowDoubleHitEffect.new()], passive),
		Ability.new("fuchsia_lf_mirror", Trigger.Type.ON_AFTER_MOVEMENT, [MirrorMoveEffect.new()], trig),
	]
...
static func assf() -> Array:
	...
	var trig: Array[int] = [AbilityComponent.Tag.TRIGGERED]
	return [
		Ability.new("fuchsia_ass_attack", Trigger.Type.ATTACK_OVERRIDE, [AssBodyShadowAttackEffect.new()], passive),
		Ability.new("fuchsia_ass_kill", Trigger.Type.ON_KILLED, [AssSpawnShadowEffect.new()], trig),
	]
...
static func aptf() -> Array:
	var on_enter: Array[int] = [AbilityComponent.Tag.ON_ENTER]
	var modifier: Array[int] = [AbilityComponent.Tag.MODIFIER]
	var trig: Array[int] = [AbilityComponent.Tag.TRIGGERED]
	return [
		Ability.new("fuchsia_apt_deploy", Trigger.Type.ON_DEPLOY, [DeploySpawnShadowEffect.new()], on_enter),
		Ability.new("fuchsia_apt_field", Trigger.Type.MOD_FIELD_INTERCEPT, [AptFieldEffect.new()], modifier, 20),
		Ability.new("fuchsia_apt_mirror", Trigger.Type.ON_AFTER_MOVEMENT, [MirrorMoveEffect.new()], trig),
	]
...
static func spf() -> Array:
	var on_enter: Array[int] = [AbilityComponent.Tag.ON_ENTER]
	return [
		Ability.new("fuchsia_sp_deploy", Trigger.Type.ON_DEPLOY, [SpGiveShadowEffect.new()], on_enter),
	]
...
static func shadow() -> Array:
	var passive: Array[int] = [AbilityComponent.Tag.PASSIVE]
	return [
		Ability.new("fuchsia_shadow_block", Trigger.Type.BLOCK_DAMAGE, [ShadowBlockEffect.new()], passive),
	]
...
static func _shadow_attack(core: GameCore, shadow_piece: PieceState) -> bool:
	...
	var targets: Array = Combat.detection(core, shadow_piece, shadow_piece.attack_types, candidates)
	if targets.is_empty():
		return false
	return Combat.launch_attack(core, linker, shadow_piece.attack_types, targets)


# --- 佈署 / 移動 效果 ---

# 佈署生鏡像於本體對稱位（可移動）。shadow_type 非空時先設定本體 shadow_attack_types（LFF="nearest"）。
class DeploySpawnShadowEffect extends AbilityEffect:
	...
	func execute(ctx: AbilityContext) -> Variant:
		...
		var pos: Vector2i = FuchsiaCards._sym(ctx.core, src.board_x, src.board_y)
		FuchsiaCards._spawn_shadow(src, src.owner, pos.x, pos.y, true)
		return null


# 本體移動後：所有可移動鏡像同步鏡射到本體新位置的對稱位（Python FuchsiaCard.after_movement）。
class MirrorMoveEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		var src: PieceState = ctx.source
		var pos: Vector2i = FuchsiaCards._sym(ctx.core, src.board_x, src.board_y)
...
class BodyPlusShadowAttackEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		var src: PieceState = ctx.source
		var core: GameCore = ctx.core
		if Combat.launch_attack(core, src, src.attack_types):
			for shadow_piece: PieceState in src.shadows:
				FuchsiaCards._shadow_attack(core, shadow_piece)
			src.hit_cards.clear()
			return true
		if not src.is_numb():
			var count: int = 0
			for shadow_piece: PieceState in src.shadows:
				if FuchsiaCards._shadow_attack(core, shadow_piece):
					count += 1
			src.hit_cards.clear()
			if count > 0:
				return true
...
class LfBodyShadowDoubleHitEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		var src: PieceState = ctx.source
		var core: GameCore = ctx.core
		if not Combat.launch_attack(core, src, src.attack_types):
			return false
		...
		for i: int in range(body_hits.size(), src.hit_cards.size()):
			...
			if body_iids.has(target.instance_id) and target.health > 0:
				...
				Combat.damage_calculate(core, target, src.damage, src, true, hurt_delay)
...
class AssBodyShadowAttackEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		var src: PieceState = ctx.source
		var core: GameCore = ctx.core
...
class AssSpawnShadowEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		var src: PieceState = ctx.source
		var victim: PieceState = ctx.target
		if victim != null:
			FuchsiaCards._spawn_shadow(src, src.owner, victim.board_x, victim.board_y, false)
		return null
...
class AuraNumbEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		var src: PieceState = ctx.source
		var core: GameCore = ctx.core
		for shadow_piece: PieceState in src.shadows:
...
class NumbTargetEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		if ctx.target != null:
...
class OccupyShadowCellsEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		var src: PieceState = ctx.source
		for shadow_piece: PieceState in src.shadows:
...
class ReleaseShadowCellsEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		var src: PieceState = ctx.source
		var core: GameCore = ctx.core
		for shadow_piece: PieceState in src.shadows:
...
class AptFieldEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		var src: PieceState = ctx.source
		var victim: PieceState = ctx.target
...
class SpGiveShadowEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		var src: PieceState = ctx.source
		var core: GameCore = ctx.core
		...
		if targets.is_empty():
			return null
		var pos: Vector2i = FuchsiaCards._sym(core, src.board_x, src.board_y)
		for ally: PieceState in Combat.detection(core, src, "farthest", targets):
			FuchsiaCards._spawn_shadow(ally, src.owner, pos.x, pos.y, false)
		return null
...
class ShadowBlockEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
