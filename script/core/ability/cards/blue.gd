...
# 藍色主題：token 引擎（3 顆換 1 抽，見 TokenEngine）。
# 每個 static func 回傳該 card_id 的 native 能力陣列（Array[Ability]）。
class_name BlueCards
extends RefCounted
...
static func registrations() -> Dictionary:
	return {
		"ADCB": Callable(BlueCards, "adcb"),
		"APB": Callable(BlueCards, "apb"),
		"TANKB": Callable(BlueCards, "tankb"),
		"HFB": Callable(BlueCards, "hfb"),
		"LFB": Callable(BlueCards, "lfb"),
		"ASSB": Callable(BlueCards, "assb"),
		"APTB": Callable(BlueCards, "aptb"),
		"SPB": Callable(BlueCards, "spb"),
	}
...
static func adcb() -> Array:
	...
	return [
		Ability.new("blue_adc_kill_token", Trigger.Type.ON_KILLED, [KillGainOnceEffect.new()], trig),
		Ability.new("blue_adc_token_draw", Trigger.Type.ON_TOKEN_DRAW, [AdcTokenDrawEffect.new()], trig),
	]
...
static func apb() -> Array:
	var trig: Array[int] = [AbilityComponent.Tag.TRIGGERED]
	return [Ability.new("blue_ap_numb_token", Trigger.Type.ON_ABILITY_HIT, [ApNumbTokenEffect.new()], trig)]


# TANKB：被攻擊後 +token_gain（got_token token_gain 次）（Python Tank.been_attacked）。
static func tankb() -> Array:
	var trig: Array[int] = [AbilityComponent.Tag.TRIGGERED]
	return [Ability.new("blue_tank_token", Trigger.Type.ON_BEEN_ATTACKED, [GainTokenGainEffect.new()], trig)]
...
static func hfb() -> Array:
	...
	return [Ability.new("blue_hf_token_dmg", Trigger.Type.ON_UPDATE, [HfTokenDamageEffect.new()], mod)]


# LFB：造成傷害後 +token_gain（got_token token_gain 次）（Python Lf.ability）。
static func lfb() -> Array:
	var trig: Array[int] = [AbilityComponent.Tag.TRIGGERED]
	return [Ability.new("blue_lf_token", Trigger.Type.ON_ABILITY_HIT, [GainTokenGainEffect.new()], trig)]


# ASSB：斬殺後 +token_gain（got_token token_gain 次）（Python Ass.killed）。
static func assb() -> Array:
	var trig: Array[int] = [AbilityComponent.Tag.TRIGGERED]
	return [Ability.new("blue_ass_token", Trigger.Type.ON_KILLED, [GainTokenGainEffect.new()], trig)]
...
static func aptb() -> Array:
	...
	var trig: Array[int] = [AbilityComponent.Tag.TRIGGERED]
	return [
		Ability.new("blue_apt_armor_dmg", Trigger.Type.ON_UPDATE, [AptArmorDamageEffect.new()], mod),
		Ability.new("blue_apt_dmg_token", Trigger.Type.ON_AFTER_DAMAGE, [AptDamageTokenEffect.new()], trig),
		Ability.new("blue_apt_after_token", Trigger.Type.ON_TOKEN_GAINED, [AptAfterTokenEffect.new()], trig),
	]
...
static func spb() -> Array:
	...
	return [Ability.new("blue_sp_deploy_burst", Trigger.Type.ON_DEPLOY, [SpDeployEffect.new()], enter)]


# --- 效果 ---

# 加 token_gain 並 got_token 一次（ADCB 斬殺用）。
class KillGainOnceEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		...
		TokenEngine.gain(core, src.owner, gain, 1)
		return true


# 加 token_gain 並 got_token token_gain 次（TANKB/LFB/ASSB 共用）。
class GainTokenGainEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		var src: PieceState = ctx.source
		var core: GameCore = ctx.core
		var gain: int = int(core.balance.param(src.card_id, "token_gain", 0))
		TokenEngine.gain(core, src.owner, gain, gain)
		return true


# APB：目標麻痺 + 加 token_gain（got_token token_gain 次）。
class ApNumbTokenEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		var src: PieceState = ctx.source
		var core: GameCore = ctx.core
		if ctx.target != null:
...
class AdcTokenDrawEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		var src: PieceState = ctx.source
		if src.is_numb():
			src.set_numb(false)
		else:
			Combat.enqueue_attack(ctx.core, src)
		return true


# HFB：extra_damage = 我方當前 token 數。
class HfTokenDamageEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		var src: PieceState = ctx.source
...
class AptArmorDamageEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		var src: PieceState = ctx.source
...
class AptDamageTokenEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		var value: int = ctx.value
		TokenEngine.gain(ctx.core, ctx.source.owner, value, value)
		return true


# APTB：我方每獲得一次 token → 自身 +1 護盾（after_token）。
class AptAfterTokenEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		ctx.source.armor += 1
		return true
...
class SpDeployEffect extends AbilityEffect:
	func execute(ctx: AbilityContext) -> Variant:
		...
		for _i in count:
			...
			Combat.damage_calculate(core, victim, dmg, src, true, 0.0)
