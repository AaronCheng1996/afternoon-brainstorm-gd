...
static func registrations() -> Dictionary:
	return {
		"ADCO": Callable(OrangeCardsV2, "adco"),
		"APO": Callable(OrangeCardsV2, "apo"),
		"TANKO": Callable(OrangeCardsV2, "tanko"),
		"HFO": Callable(OrangeCardsV2, "hfo"),
		"LFO": Callable(OrangeCardsV2, "lfo"),
		"ASSO": Callable(OrangeCardsV2, "asso"),
		"APTO": Callable(OrangeCardsV2, "apto"),
		"SPO": Callable(OrangeCardsV2, "spo"),
	}
...
static func adco() -> Array:
	...
	return [
		AbilityV2.new("orange_adc_attack_move", TriggerV2.Type.ATTACK_OVERRIDE, [AttackThenMoveEffect.new()], passive),
		AbilityV2.new("orange_adc_move_attack", TriggerV2.Type.ON_AFTER_MOVEMENT, [AfterMoveAttackEffect.new()], trig),
	]
...
static func apo() -> Array:
	var trig: Array[int] = [AbilityComponentV2.Tag.TRIGGERED]
	return [
		AbilityV2.new("orange_ap_numb", TriggerV2.Type.ON_ABILITY_HIT, [ApNumbEffect.new()], trig),
		AbilityV2.new("orange_ap_moveo", TriggerV2.Type.ON_REFRESH, [AddMoveoEffect.new()], trig),
	]
...
static func tanko() -> Array:
	var trig: Array[int] = [AbilityComponentV2.Tag.TRIGGERED]
	return [AbilityV2.new("orange_tank_moveo", TriggerV2.Type.ON_BEEN_ATTACKED, [AddMoveoEffect.new()], trig)]
...
static func hfo() -> Array:
	...
	return [
		AbilityV2.new("orange_hf_attack_move", TriggerV2.Type.ATTACK_OVERRIDE, [AttackThenMoveEffect.new()], passive),
		AbilityV2.new("orange_hf_after_move", TriggerV2.Type.ON_AFTER_MOVEMENT, [HfAfterMoveEffect.new()], trig),
		AbilityV2.new("orange_hf_settle", TriggerV2.Type.ON_SETTLE, [HfSettleEffect.new()], mod),
	]
...
static func lfo() -> Array:
	...
	var trig: Array[int] = [AbilityComponentV2.Tag.TRIGGERED]
	return [
		AbilityV2.new("orange_lf_attack_move", TriggerV2.Type.ATTACK_OVERRIDE, [AttackThenMoveEffect.new()], passive),
		AbilityV2.new("orange_lf_move_strike", TriggerV2.Type.ON_AFTER_MOVEMENT, [LfMoveStrikeEffect.new()], trig),
	]
...
static func asso() -> Array:
	...
	var mod: Array[int] = [AbilityComponentV2.Tag.MODIFIER]
	return [
		AbilityV2.new("orange_ass_after_move", TriggerV2.Type.ON_AFTER_MOVEMENT, [AssAfterMoveEffect.new()], trig),
		AbilityV2.new("orange_ass_kill", TriggerV2.Type.ON_KILLED, [AssKilledEffect.new()], trig),
		AbilityV2.new("orange_ass_settle", TriggerV2.Type.ON_SETTLE, [AssSettleEffect.new()], mod),
	]
...
static func apto() -> Array:
	var trig: Array[int] = [AbilityComponentV2.Tag.TRIGGERED]
	return [
		AbilityV2.new("orange_apt_after_move", TriggerV2.Type.ON_AFTER_MOVEMENT, [AptAfterMoveEffect.new()], trig),
		AbilityV2.new("orange_apt_move_armor", TriggerV2.Type.ON_MOVE_BROADCAST, [AptMoveBroadcastEffect.new()], trig),
	]
...
static func spo() -> Array:
	var trig: Array[int] = [AbilityComponentV2.Tag.TRIGGERED]
	return [AbilityV2.new("orange_sp_move_strike", TriggerV2.Type.ON_MOVE_BROADCAST, [SpMoveBroadcastEffect.new()], trig)]
...
class AttackThenMoveEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var src: PieceState = ctx.source
		var ok: bool = CombatV2.launch_attack(ctx.core, src, src.attack_types)
		if ok:
			src.set_moving(true)
		return ok
...
class AfterMoveAttackEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var src: PieceState = ctx.source
...
# APO：攻擊附帶麻痺目標。
class ApNumbEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		if ctx.target != null:
...
class AddMoveoEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
...
class HfAfterMoveEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var src: PieceState = ctx.source
...
class LfMoveStrikeEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var src: PieceState = ctx.source
		...
		for target: PieceState in CombatV2.detection(core, src, "nearest", pool):
			CombatV2.damage_calculate(core, target, src.damage, src, true, 0.0)
		return true


# ASSO：移動後進怒氣。
class AssAfterMoveEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
...
class AssSettleEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
...
class AptAfterMoveEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var src: PieceState = ctx.source
...
class AptMoveBroadcastEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var src: PieceState = ctx.source
...
class SpMoveBroadcastEffect extends AbilityEffectV2:
	func execute(ctx: AbilityContextV2) -> Variant:
		var src: PieceState = ctx.source
		var core: GameCore = ctx.core
		var mover: PieceState = ctx.target
		if mover != null and mover.owner == src.owner:
			...
			for target: PieceState in CombatV2.detection(core, src, "farthest", core.get_enemies_of(src.owner)):
