...
func _is_silenced(p: PieceState) -> bool:
	return p.abilities != null and p.abilities.silenced_tags.has(AbilityComponent.Tag.FACTION)


func run(t: Object) -> void:
	var cores: Array = []
	const FACTION := AbilityComponent.Tag.FACTION
	...
	apt10.abilities.run(Trigger.Type.ON_DEPLOY, AbilityContext.new(c10, apt10, null, 0, {}))
	...
	apt11.abilities.run(Trigger.Type.ON_ABILITY_HIT, AbilityContext.new(c11, apt11, null, 0, {}))
	...
	apt12.abilities.run(Trigger.Type.ON_ABILITY_HIT, AbilityContext.new(c12, apt12, null, 0, {}))
