...
func _run_token_draw(core: GameCore, piece: PieceState) -> void:
	piece.abilities.run(TriggerV2.Type.ON_TOKEN_DRAW, AbilityContextV2.new(core, piece, null, 0, {}))
...
func run(t: Object) -> void:
	...
	CombatV2.attack(c8, ap8)   # +2 → 4；got_token#1 換抽剩 1；got_token#2 不足
	...
	CombatV2.damage_calculate(c9, tank9, 1, atk9, false, 0.0)
	...
	hf10.abilities.run(TriggerV2.Type.ON_UPDATE, AbilityContextV2.new(c10, hf10, null, 0, {}))
	...
	hf11.abilities.run(TriggerV2.Type.ON_UPDATE, AbilityContextV2.new(c11, hf11, null, 0, {}))
	...
	apt15.abilities.run(TriggerV2.Type.ON_TOKEN_GAINED, AbilityContextV2.new(c15, apt15, null, 0, {}))
	...
	apt16.armor = div16 * 2
	apt16.abilities.run(TriggerV2.Type.ON_UPDATE, AbilityContextV2.new(c16, apt16, null, 0, {}))
	t.eq(apt16.extra_damage, 2, "APTB extra_damage = armor//divisor")
	var tok16_before: int = c16.players_token["player1"]
	CombatV2.attack(c16, apt16)
	...
	c17.pending_attacks.append(CombatV2.make_request(enemy17))   # 佈署後應被清空
